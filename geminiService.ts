...
export async function analyzeLessonSource(lessonId: string, title: string, description: string, language: 'ne' | 'en'): Promise<LessonContent> {
  // 1. Check Cache First (Database Service handles Firestore + LocalStorage fallback)
  ...
  const prompt = `
    ...
    Objective: Create a high-quality lesson on "${title}" (${description}) for learners in ${langLabel}.
  ...
  try {
    ...
    if (error?.message?.includes('403') || error?.status === 403) {
      console.warn("Gemini API Permission Denied. Check if your API Key has access to 'gemini-3-flash-preview' and Google Search tool.");
    }
